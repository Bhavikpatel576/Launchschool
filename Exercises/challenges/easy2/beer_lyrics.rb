#each phrase is seperated by an additional new line
#once the number of bottles gets down to 2, the wording that we have to 
#use changes from beers to beer (singular)
#zero bottles has it's own phrase
...
class BeerSong
	def verse(*lines)
		...
		lines.each do |v|
			if v > 0
				output << "#{v} bottle of beer on the wall, #{v} bottle of beer.\n" \
