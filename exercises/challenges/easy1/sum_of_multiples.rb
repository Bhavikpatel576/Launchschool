#create a program that can find the sum of multiples, exclusive of the number
...
class SumOfMultiples
	...
	def to(max)
		array = [0]
		1.upto(max) do |value|
			array << value if multiple?(value, multiples) && value != max
		end
		array.reduce(&:+)
	end

	def multiple?(number, multiples)
		multiples.any? {|v| number % v == 0}
	end
