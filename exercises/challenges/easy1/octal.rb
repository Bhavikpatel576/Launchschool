...
class Octal
	...
	def test_range(array)
		return nil if array.any? { }
	end

	def to_decimal
		num_string = @number.to_s.chars.map(&:to_i).reverse
		return 0 unless test_range(num_string)
		value_array = num_string.map.with_index do |value, index|
	  	value*(BASE**index)
	  end
	  value_array.reduce(&:+)
	end 
