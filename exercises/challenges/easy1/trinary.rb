...
class Trinary
	...
	def to_decimal
		number =~ INVALID ? 0 : calculate
	end

	private
	def calculate
		decimal = 0
		number.chars.reverse.each_with_index do |value, index|
			decimal += value.to_i*(3**index)
		end
		decimal
	end
end
...
class TestTrinary < Minitest::Test
	def test_trainary_1_is_decimal_1
